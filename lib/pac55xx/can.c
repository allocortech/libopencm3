...
void can_filter_dual(uint32_t canport, uint32_t id1, uint32_t id1_mask,
	...
	/* set value */
	uint32_t word = ((id1 << 21) & CAN_ACR_DUAL_ID1) | ((id2 << 5) & CAN_ACR_DUAL_ID2)
					| ((db << 12) & CAN_ACR_DUAL_DB_UPPER) | (db & CAN_ACR_DUAL_DB_LOWER);
	CAN_ACR(canport) = __builtin_bswap32(word);
	/* set mask */
	word = ((~id1_mask << 21) & CAN_ACR_DUAL_ID1) | ((~id2_mask << 5) & CAN_ACR_DUAL_ID2)
...
void can_filter_single_std(uint32_t canport, uint32_t id, uint32_t id_mask,
	...
	/* set value */
	uint32_t word = ((id << 21) & CAN_ACR_SINGLE_STD_ID) | ((db1 << 8) & CAN_ACR_SINGLE_STD_DB1)
					| ((db2 << 0) & CAN_ACR_SINGLE_STD_DB2);
	CAN_ACR(canport) = __builtin_bswap32(word);
	/* set mask */
	word = ((~id_mask << 21) & CAN_ACR_SINGLE_STD_ID) |
	CAN_ACR_SINGLE_STD_RTR |
	CAN_ACR_DUAL_DB_UPPER | ((~db1_mask << 8) & CAN_ACR_SINGLE_STD_DB1)
...
*/
int can_transmit_std(uint32_t canport, uint32_t id, bool rtr, uint8_t length,
				const uint8_t *data) {
	if (length > 8) {
		return -1;
	}
...
@returns int 0 on success, or -1 on failure.
*/
int can_transmit_ext(uint32_t canport, uint32_t id, bool rtr, uint8_t length,
				const uint8_t *data) {
	if (length > 8) {
		return -1;
	}
...
@return 1 on success
*/
int can_receive(uint32_t canport, uint32_t *id, bool *ext, bool *rtr, uint8_t *length,
				uint8_t *can_rx_data) {
